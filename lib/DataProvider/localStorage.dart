...
class LocalStorage {
  ...
  Future<void> deleteAllRecord() async {
    var store = StoreRef<String, List<Object?>>.main();
    Database localDB = await LocalStorage.instance.database;
    await store.record(DatabaseRecords.groups).delete(localDB);
    await store.record(DatabaseRecords.friends).delete(localDB);
    await store.record(DatabaseRecords.activity).delete(localDB);
  }
